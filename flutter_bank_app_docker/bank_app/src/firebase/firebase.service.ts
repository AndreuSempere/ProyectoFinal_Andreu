...
export class FirebaseService {
  ...
  constructor() {
    try {
      if (!process.env.STORAGE_BUCKET_FIREBASE) {
        this.logger.error(
          'STORAGE_BUCKET_FIREBASE no está definido en las variables de entorno',
        );
        console.error(
          'STORAGE_BUCKET_FIREBASE no está definido en las variables de entorno',
        );
      ...
      console.log(
        'Firebase inicializado correctamente con bucket:',
        bucketName,
      );
      this.logger.log(
        `Firebase inicializado correctamente con bucket: ${bucketName}`,
      );
  ...
  async uploadPdfToStorage(
    ...
    try {
      ...
      this.logger.log(
        `PDF subido correctamente a Firebase Storage: ${publicUrl}`,
      );
      return publicUrl;
    } catch (error) {
      this.logger.error(
        `Error al subir PDF a Firebase Storage: ${error.message}`,
      );
      throw new Error(
        `Error al subir PDF a Firebase Storage: ${error.message}`,
      );
