...
describe('Credit_CardService', () => {
  ...
  const MockCreditCardRepository = {
    ...
    findOne: jest.fn(({ where }) => {
      if (where.id_tarjeta === 1) {
        ...
        where.numero_tarjeta === 2345678901234567 &&
        where.id_tarjeta !== 2
      ) {
  ...
  describe('getidCreditCard', () => {
    ...
    it('should throw an exception when credit card is not found', async () => {
      ...
      await expect(creditCardService.getidCreditCard(999)).rejects.toThrow(
        HttpException,
      );
    });
  });
  ...
  describe('getCreditCard', () => {
    ...
    it('should throw an exception when credit card is not found', async () => {
      ...
      await expect(
        creditCardService.getCreditCard(9999999999999999),
      ).rejects.toThrow();
    });
  });
  ...
  describe('createCreditCard', () => {
    it('should create a new credit card', async () => {
      const result =
        await creditCardService.createCreditCard(createCreditCardDto);
  ...
  describe('updateCreditCard', () => {
    ...
    it('should throw an exception when credit card is not found', async () => {
      ...
      await expect(
        creditCardService.updateCreditCard(999, updateCreditCardDto),
      ).rejects.toThrow(HttpException);
    });
    ...
    it('should throw an exception when new card number already exists', async () => {
      ...
      await expect(creditCardService.updateCreditCard(1, dto)).rejects.toThrow(
        HttpException,
      );
    });
  });
  ...
  describe('deleteCreditCard', () => {
    ...
    it('should throw an exception when credit card is not found', async () => {
      await expect(creditCardService.deleteCreditCard(999)).rejects.toThrow(
        HttpException,
      );
    });
  });
