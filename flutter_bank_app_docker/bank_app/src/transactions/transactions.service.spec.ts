...
describe('TransactionsService', () => {
  ...
  const mockQueryBuilder = {
    ...
    getMany: jest.fn().mockResolvedValue([transactionsArray[0], transactionsArray[1]])
  };
  ...
  const MockAccountsRepository = {
    findOne: jest.fn().mockImplementation(({ where }) => {
      if (where.id_cuenta === 1 || where[0]?.id_cuenta === 1 || where[0]?.numero_cuenta === 1234567890123456) {
        return Promise.resolve(sourceAccount);
      } else if (where.id_cuenta === 2 || where[0]?.id_cuenta === 2 || where[0]?.numero_cuenta === 2345678901234567) {
  ...
  const MockFirebaseService = {
    ...
    uploadPdfToStorage: jest.fn().mockResolvedValue('https://example.com/receipt.pdf')
  };
  ...
  describe('getTransaction', () => {
    it('should return a transaction by id', async () => {
      ...
      expect(MockTransactionsRepository.findOneBy).toHaveBeenCalledWith({ id_transaction: 1 });
  ...
  describe('getTransactionsByAccountId', () => {
    it('should return transactions for a specific account', async () => {
      ...
      expect(mockQueryBuilder.innerJoinAndSelect).toHaveBeenCalledWith('transaction.account', 'account');
      expect(mockQueryBuilder.where).toHaveBeenCalledWith('account.id_cuenta = :accountId', { accountId: 1 });
      expect(mockQueryBuilder.orderBy).toHaveBeenCalledWith('transaction.id_transaction', 'DESC');
  ...
  describe('createTransaction', () => {
    it('should create a transaction successfully', async () => {
      const result = await transactionsService.createTransaction(createTransactionDto);
    ...
    it('should throw an exception when source account is not found', async () => {
      ...
      await expect(transactionsService.createTransaction({
