...
export class TransactionsController {
  ...
  async getTransactionAll() {
    try {
      ...
      throw new BadRequestException('Error fetching transactions' + err);
    }
  }
  ...
  @ApiOperation({
    ...
    description: 'Fetches all transactions associated with a user ID.',
  })
  @ApiResponse({
    status: 200,
    description: 'Successfully fetched the user transactions.',
  })
  @ApiResponse({
    status: 404,
  ...
  async getTransactionsByAccountId(@Param('id') id: string) {
    ...
    const transactions =
      await this.transactionsService.getTransactionsByAccountId(userId);
  ...
  async createTransaction(@Body() createTransactionDto: CreateTransactionDto) {
    try {
      ...
      throw new BadRequestException('Failed to create transaction' + err);
    }
  }

  @Post('/bizum/')
  @ApiOperation({
    summary: 'Create a Bizum transaction',
    description: 'Creates a Bizum transaction based on the provided data.',
  })
  @ApiResponse({
    status: 201,
    description: 'Successfully created the Bizum transaction.',
  })
  @ApiResponse({ status: 400, description: 'Bad request, invalid data.' })
  async bizumTransaction(@Body() createTransactionDto: CreateTransactionDto) {
    try {
      return await this.transactionsService.bizumTransaction(
        createTransactionDto,
      );
    } catch (err) {
      throw new BadRequestException('Failed to create Bizum transaction' + err);
    }
  }
